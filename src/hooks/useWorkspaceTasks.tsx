...
export const useWorkspaceTasks = (workspaceId: string) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const taskDetails: TaskDetails = {
          ...
          input_format: 'API Specification',
          input_samples: [{ id: 'input1', value: ramlTask.description, isValid: true }],
          output_samples: [],
  ...
  const fetchTaskDetails = async (taskId: string) => {
    ...
    try {
      // First try to get RAML task details
      ...
      // Next try to get integration task details
      ...
      console.log('Task not found in RAML or integration tasks, checking regular tasks');
  ...
  const saveRamlTask = async (ramlData: {
    ...
    try {
      // Generate a unique task_id if not provided
      const taskId = ramlData.task_id || `RAML-${crypto.randomUUID().substring(0, 8).toUpperCase()}`;
      ...
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          task_id: taskId,
          task_name: ramlData.task_name || ramlData.api_name,
          description: ramlData.description || `RAML specification for ${ramlData.api_name}`,
          ...
          media_types: JSON.parse(JSON.stringify(ramlData.media_types)) as Json,
          protocols: JSON.parse(JSON.stringify(ramlData.protocols)) as Json,
          types: JSON.parse(JSON.stringify(ramlData.types)) as Json,
          endpoints: JSON.parse(JSON.stringify(ramlData.endpoints)) as Json
        }])
        .select();
      ...
      // Refresh the task list with the current workspace
      await fetchWorkspaceTasks();
      console.log('RAML task saved:', data);
      return data;
    } catch (err: any) {
