...
The script must be optimized for production use in MuleSoft Anypoint Studio.`;
    ...
    const response = await fetch('https://api.mistral.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a DataWeave expert that provides only clean, executable DataWeave code without any additional text, comments, or formatting.'
          },
          {
            role: 'user',
            content: userPrompt
    ...
    for(let i = lines.length - 1; i >= 0; i--){
      ...
      if (line.startsWith('Note:') || line.startsWith('Explanation:') || line.toLowerCase().includes('this script') || line.startsWith('In this') || line.startsWith('The script')) {
        endIndex = i;
      } else {
        break;
      }
    ...
    const cleanedScript = generatedScript.replace(/\/\*[\s\S]*?\*\/$/gm, '') // Remove trailing multiline comments
